...
module.exports.load = async function(app, db) {
    app.get("/setcoins", async (req, res) => {
        ...
        log(`set coins`, `${req.session.userinfo.username} set the coins of the user with the ID \`${id}\` to \`${coins}\`.`)
    ...
    app.get("/addcoins", async (req, res) => {
        ...
        let successredirect = theme.settings.redirect.setcoins || "/";
        log(`add coins`, `${req.session.userinfo.username} added \`${req.query.coins}\` coins to the user with the ID \`${id}\`'s account.`)
        res.redirect(successredirect + "?err=none");
    });
    ...
    app.get("/setresources", async (req, res) => {
        ...
        if (req.query.ram || req.query.disk || req.query.cpu || req.query.servers) {
            ...
            log(`set resources`, `${req.session.userinfo.username} set the resources of the user with the ID \`${id}\` to:\`\`\`servers: ${serversstring}\nCPU: ${cpustring}%\nMemory: ${ramstring} MB\nDisk: ${diskstring} MB\`\`\``)
    ...
    app.get("/setplan", async (req, res) => {
        ...
        if (!req.query.package) {
            ...
            log(`set plan`, `${req.session.userinfo.username} removed the plan of the user with the ID \`${req.query.id}\`.`)
            return res.redirect(successredirect + "?err=none");
        } else {
            ...
            log(`set plan`, `${req.session.userinfo.username} set the plan of the user with the ID \`${req.query.id}\` to \`${req.query.package}\`.`)
            return res.redirect(successredirect + "?err=none");
        }
    });
    ...
    app.get("/create_coupon", async (req, res) => {
        ...
        log(`create coupon`, `${req.session.userinfo.username} created the coupon code \`${code}\` which gives:\`\`\`coins: ${coins}\nMemory: ${ram} MB\nDisk: ${disk} MB\nCPU: ${cpu}%\nServers: ${servers}\`\`\``)
    ...
    app.get("/revoke_coupon", async (req, res) => {
        ...
        log(`revoke coupon`, `${req.session.userinfo.username} revoked the coupon code \`${code}\`.`)
    ...
    app.get("/remove_account", async (req, res) => {
        ...
        log(`remove account`, `${req.session.userinfo.username} removed the account with the ID \`${discordid}\`.`)
    ...
    app.get("/getip", async (req, res) => {
        ...
        log(`view ip`, `${req.session.userinfo.username} viewed the IP of the account with the ID \`${req.query.id}\`.`)
        console.log(`${req.session.userinfo.username} viewed the IP of the account with the ID \`${req.query.id}\`.`)
