...
module.exports.load = async function(app, db) {
  ...
  app.get("/buyram", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if (amount < 1 || amount > 10) return res.send("amount is out of range");
      ...
      if (newram > settings.storelimits.ram) return res.send("You reached the maximum RAM limit!");
      if (newusercoins === 0) {
      ...
      log(
        `Resources Purchased`,
        `${req.session.userinfo.username}#${req.session.userinfo.discriminator} bought ${per} MB of RAM from the store for \`${cost}\` Credits.`
      );
  ...
  async function enabledCheck(req, res) {
    let newsettings = JSON.parse(fs.readFileSync("./databases/settings.json").toString());
