...
async function users() {
  ...
  function i() {
    ...
    const m = a.slice(k, l);
    c.innerHTML = m.map(i => `
      <tr onclick="render('admin/users/view/${i.hcx.id}')" class="cursor-pointer">
        <td class="p-4 transition-all duration-300 ease-in-out text-left">
          <img class="object-cover w-12 rounded-xl" src="${i.hcx.avatar}">
        </td>
        <td class="p-4 transition-all duration-300 ease-in-out text-left">
          ${i.hcx.name?.first} ${i.hcx.name?.last}
        </td>
        <td class="p-4 transition-all duration-300 ease-in-out text-left">
          ${i.hcx.username}
        </td>
        ...
        <td class="p-4 transition-all duration-300 ease-in-out text-right capitalize">
          ${i.hcx.permissions.roles}
        </td>
        <td class="p-4 transition-all duration-300 ease-in-out text-right">
          ${i.hcx.permissions.level}
        </td>
      </tr>
    `).join('');
  }
...
const es = ["appname", "applogo", "appbanner", "appdescription", "logorotate", "rotatespeed", "seotitle", "seoimage", "seodescription", "seokeys", "subdomainsDefault", "subdomainsStatus", "CFKey", "CFEmail"];
const originalValues = {};
function ge(e) {
  return document.getElementById(e);
}
ge("applogo").addEventListener("input", function () {
  ge("logoHolder").src = ge("applogo").value
});
ge("seoimage").addEventListener("input", function () {
  ge("seoimg").src = ge("seoimage").value
});
ge("seodescription").addEventListener("input", function () {
  ge("seodes").innerText = ge("seodescription").value
});
ge("seotitle").addEventListener("input", function () {
  ge("seoTitle").innerText = ge("seotitle").value
});
async function loadSettings() {
  check()
  let a = await fetch(`/api/admin/settings/subdomains?type=full`)
  let b = await a.json()
  if (b.success === true) {
    sv("subdomainsStatus", b.data.enabled)
    sv("subdomainsDefault", parseInt(b.data.default))
    sv("CFEmail", b.data.cloudflare.email)
    sv("CFKey", b.data.cloudflare.key)
    sv("CFZones", b.data.cloudflare.zones.join(", "))
  }
}
function sv(a, b) {
...
function check() {
  for (const e of es) {
    const element = ge(e);
    originalValues[e] = element.value || element.checked;
    element.addEventListener("input", function () {
      ge("savealert").style.transition = "opacity 0.5s";
      ge("savealert").style.opacity = "1";
      if (element.value === originalValues[e]) {
        ge("savealert").style.transition = "opacity 0.5s";
        ge("savealert").style.opacity = "0";
      }
    });
...
async function submitSettings() {
  let a = await fetch(`/api/admin/settings`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify({
      name: ge("appname").value,
      logo: ge("applogo").value,
      speed: ge("rotatespeed").value,
      rotate: ge("logorotate").value,
      banner: ge("appbanner").value,
      description: ge("appdescription").value,
      seoImage: ge("seoimage").value,
      seoDes: ge("seodescription").value,
      seoTitle: ge("seotitle").value,
      seoKeys: ge("seokeys").value,
    })
  });
  let b = await a.json()
  if (b.success == true) {
  ...
  let c = await fetch(`/api/admin/settings/subdomains?type=full`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify({
      enabled: ge("subdomainsStatus").value,
      default: parseInt(ge("subdomainsDefault").value),
      cloudflare: {
        key: ge("CFKey").value,
        email: ge("CFEmail").value,
        zones: ge("CFZones").value.split(',')
      }
    })
  });
  let d = await c.json()
...
async function appLicense() {
  let a = await fetch('/api/app/license', {
    method: "POST",
  });
  let b = await a.json()
  if (b.success == true) {
    sv("licenseKey", b.data)
  }
}
async function submitLicense() {
  let a = await fetch('/api/app/license', {
    ...
    body: JSON.stringify({ "key": gv("licenseKey") })
  });
  let b = await a.json()
  if (b.success == true) {
    toastr.success("Success", b.message)
  }
}
>>>>>>> 35d272e (25-06.app.public)
