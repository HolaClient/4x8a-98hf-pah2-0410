...
module.exports.load = async function(app, db) {
  ...
  app.get('/api/admin/servers', async (req, res) => {
    ...
    const servers = json.data.map((server) => ({
      ...
      backups: server.attributes.limits.backups,
      databases: server.attributes.limits.databases,
      allocations: server.attributes.limits.allocations
    }));
    res.json(servers);
  });
  ...
  app.get('/api/admin/servers/details', async (req,res) => {
    ...
    await fetch(settings.pterodactyl.domain + `/api/application/servers/${req.query.id}`, {
      ...
      .then(json => {
        ...
        server.backups = json.attributes.limits.backups,
        server.databases = json.attributes.limits.databases,
        server.allocations = json.attributes.limits.allocations
      })
