...
module.exports.load = async function (app, db) {
  ...
  app.get("/arc-sw.js", async (req, res) => {
    try {
      if (settings.earn.arcio.enabled == true) {
        ...
        let theme = appjs.get(req);
        page.render(theme.settings.errors[403], req, res);
      }
    } catch (error) {
  ...
  app.get("/arcioerror", core.auth, async (req, res) => {
    let theme = appjs.get(req);
    res.redirect(theme.settings.redirect.arcioerror + (req.query.err ? ("?arcioerr=" + req.query.err) : ""));
  });

  app.ws("/" + settings.earn.arcio["afk page"].path, async (ws, req) => {
    try {
      ...
      if (settings.earn.arcio.enabled !== true) return ws.close();
      if (settings.earn.arcio["afk page"].enabled !== true) return ws.close();
      if (currentlyonpage[req.session.userinfo.hcid]) return ws.close();

      currentlyonpage[req.session.userinfo.hcid] = true;

      let coinloop = setInterval(
        async function () {
          let usercoins = await db.get("coins", req.session.userinfo.hcid);
          usercoins = usercoins ? usercoins : 0;
          usercoins = usercoins + settings.earn.arcio["afk page"].coins;
          if (usercoins > 999999999999999) return ws.close();
          await db.set("coins", req.session.userinfo.hcid, usercoins);
        }, settings.earn.arcio["afk page"].every * 1000
      );

      ws.onclose = async () => {
        clearInterval(coinloop);
        delete currentlyonpage[req.session.userinfo.hcid];
      };
    } catch (error) {
